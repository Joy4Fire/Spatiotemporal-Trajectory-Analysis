...
typedef enum __mp_WindowMove_Type {
	MOVE_UP,  
	MOVE_DOWN,  
	MOVE_LEFT,  
	MOVE_RIGHT
...
class _declspec(dllexport) mpLayer;
class _declspec(dllexport) mpFX;
class _declspec(dllexport) mpFY;
class _declspec(dllexport) mpFXY;
class _declspec(dllexport) mpFXYVector;
class _declspec(dllexport) mpScaleX;
class _declspec(dllexport) mpScaleY;
class _declspec(dllexport) mpWindow;
class _declspec(dllexport) mpText;
class _declspec(dllexport) mpPrintout;
...
typedef enum __mp_Layer_Type {
	...
	mpLAYER_NO_Legend  //add by nowen
} mpLayerType;
...
*/
class _declspec(dllexport) mpLayer : public wxObject
{
public:
    mpLayer();
    ...
    */
	...
	void SetLayerType(mpLayerType Type) { m_type = Type; };
	//-------------------------------------------------------------------------------
...
*/
class _declspec(dllexport) mpInfoLayer : public mpLayer
{
public:
    /** Default constructor. */
    mpInfoLayer();
...
class _declspec(dllexport) mpInfoCoords : public mpInfoLayer
{
public:
    /** Default constructor */
    mpInfoCoords();
...
/** @class mpInfoText
*/
class _declspec(dllexport) mpInfoText : public mpInfoLayer
{
public:
	/** Default constructor */
	mpInfoText(wxString name = wxT("Title"));
	/** Complete constructor, setting initial rectangle and background brush.
	@param rect The initial bounding rectangle.
	@param brush The wxBrush to be used for box background: default is transparent */
	mpInfoText(wxRect rect, wxString name = wxT("Title"), const wxBrush* brush = wxTRANSPARENT_BRUSH);
...
protected:
	wxString m_content; //!< string holding the text to be drawn.
};


/** @class mpInfoLine
*/
class _declspec(dllexport) mpInfoLine : public mpInfoLayer
{
public:
	/** Default constructor */
	mpInfoLine();
	...
	@param rect The initial bounding rectangle.
	@param brush The wxBrush to be used for box background: default is transparent */
	mpInfoLine(wxRect rect, double length, wxPen pen = wxPen(*wxGREEN,1,wxLONG_DASH));
...
protected:
	double m_length; //!< string holding the text to be drawn.
};
//////////////////////////////////////////////////////////////////////////
...
class _declspec(dllexport) mpInfoLegend : public mpInfoLayer
{
public:
    /** Default constructor */
    mpInfoLegend();
...
*/
class _declspec(dllexport) mpFX : public mpLayer
{
public:
    /** @param name  Label
...
*/
class _declspec(dllexport) mpFY : public mpLayer
{
public:
    /** @param name  Label
...
*/
class _declspec(dllexport) mpFXY : public mpLayer
{
public:
    /** @param name  Label
...
*/
class _declspec(dllexport) mpProfile : public mpLayer
{
public:
    /** @param name  Label
        @param flags Label alignment, pass one of #mpALIGN_BOTTOM, #mpALIGN_CENTER, #mpALIGN_TOP.
...
*/
class _declspec(dllexport) mpScaleX : public mpLayer
{
public:
    ...
    */
	...
	//----------↓↓↓↓↓↓↓↓↓↓↓－edit by nowen－↓↓↓↓↓↓↓↓↓↓↓----------
	//设置标注的格式，例如：SetFmt("%.f"),SetFmt("%d");
	void SetFmt(wxString fmt) { m_fmt = fmt; };
	//-------------------------------------------------------------------------------

protected:
	//----------↓↓↓↓↓↓↓↓↓↓↓－edit by nowen－↓↓↓↓↓↓↓↓↓↓↓----------

	wxString m_fmt;//标注的格式
	//-------------------------------------------------------------------------------
...
/** Plot layer implementing a y-scale ruler.
    ...
    adjusts to view and zoom factor.
*/
class _declspec(dllexport) mpScaleY : public mpLayer
{
public:
    /** @param name Label to plot by the ruler
        @param flags Set position of the scale respect to the window.
...
adjusts to view and zoom factor.
*/
class _declspec(dllexport) mpSubScaleY : public mpLayer
{
public:
	/** @param name Label to plot by the ruler
	@param flags Set position of the scale respect to the window.
	@param ticks Select ticks or grid. Give TRUE for drawing axis ticks, FALSe for drawing the grid */
	mpSubScaleY(wxString name = wxT("Y"), int flags = mpALIGN_RIGHT, bool ticks = true);
...
*/
class _declspec(dllexport) mpWindow : public wxScrolledWindow
{
public:
    mpWindow() {}
...
//     wxCoord y2p(double y, bool drawOutside = true); // { return (wxCoord) ( (m_posY-y) * m_scaleY); }
    ...
    */
	...
	//nowen
	void WindowMove(mpMoveType MoveType)
	{
		switch ( MoveType)
		{
		case MOVE_UP:		m_desiredYmax += Increase; m_desiredYmin += Increase; break;
		case MOVE_DOWN:		m_desiredYmax -= Increase; m_desiredYmin -= Increase; break;
		case MOVE_LEFT:		m_desiredXmax -= Increase; m_desiredXmin -= Increase; break;
		case MOVE_RIGHT:	m_desiredXmin += Increase; m_desiredXmin += Increase; break;
		}
		UpdateAll();
	}
	...
	{
		double tmp;
		CHANGE(m_minX,m_minX_previous,tmp);
		CHANGE(m_maxX,m_maxX_previous,tmp);
		CHANGE(m_minY,m_minY_previous,tmp);
		CHANGE(m_maxY,m_maxY_previous,tmp);
		Refresh( FALSE );
	}
...
//     virtual void DoPrepareDC(wxDC& dc);
	...
	//nowen
	double m_minX_previous, m_maxX_previous, m_minY_previous, m_maxY_previous;
...
*/
class _declspec(dllexport) mpFXYVector : public mpFXY
{
public:
    /** @param name  Label
        @param flags Label alignment, pass one of #mpALIGN_NE, #mpALIGN_NW, #mpALIGN_SW, #mpALIGN_SE.
...
factors.
*/
class _declspec(dllexport) mpText : public mpLayer
{
public:
...
*/
class _declspec(dllexport) mpPrintout : public wxPrintout
{
public:
...
/** This virtual class represents objects that can be moved to an arbitrary 2D location+rotation.
  ...
  */
class _declspec(dllexport) mpMovableObject : public mpLayer
{
public:
...
/** A 2D ellipse, described by a 2x2 covariance matrix.
  ...
  */
class _declspec(dllexport) mpCovarianceEllipse : public mpMovableObject
{
public:
    /** Default constructor.
...
/** An arbitrary polygon, descendant of mpMovableObject.
  ...
  */
class _declspec(dllexport) mpPolygon : public mpMovableObject
{
public:
    /** Default constructor.
      */
	...
	/*将void setPoints(
		...
		bool                   closedShape=true );改为↓↓↓为了跟mpFXYVector统一
    */
    //-------------------------------------------------------------------------------
...
/** This virtual class represents objects that can be moved to an arbitrary 2D location+rotation.
  ...
  *  the object updates its shape in m_shape_xs & m_shape_ys.
  */
class _declspec(dllexport) mpBitmapLayer : public mpLayer
{
public:
    /** Default constructor.
      */
