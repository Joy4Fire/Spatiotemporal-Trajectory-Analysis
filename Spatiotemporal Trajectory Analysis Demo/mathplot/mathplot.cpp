...
//nowen
//////////////////////////////////////////////////////////////////////////
mpInfoText::mpInfoText(wxString name/* = wxT("Title")*/) : mpInfoLayer()
{
	m_content = name;
}

mpInfoText::mpInfoText(wxRect rect,  wxString name/* = wxT("Title")*/, const wxBrush* brush/* = wxTRANSPARENT_BRUSH*/) : mpInfoLayer(rect, brush)
{
	m_content = name;
}

mpInfoText::~mpInfoText()
{

}


void mpInfoText::Plot(wxDC & dc, mpWindow & w)
{
	if (m_visible) {
		// Adjust relative position inside the window
		int scrx = w.GetScrX();
...
mpInfoLine::mpInfoLine() : mpInfoLayer()
{
}

mpInfoLine::mpInfoLine(wxRect rect, double length, wxPen pen/* = wxPen(*wxGREEN,1,wxLONG_DASH)*/) : mpInfoLayer(rect, wxTRANSPARENT_BRUSH)
{
	m_length = length;
	m_pen = pen;
}
...
void mpInfoLine::Plot(wxDC & dc, mpWindow & w)
{
	if (m_visible) {
		// Adjust relative position inside the window
		int scrx = w.GetScrX();
		...
		if (m_dim.width < textX + 10) m_dim.width = textX + 10;
		if (m_dim.height < textY + 10) m_dim.height = textY + 10;
		dc.DrawLine(m_dim.x, m_dim.y, m_dim.x, m_dim.y + m_length);
...
IMPLEMENT_CLASS(mpSubScaleY, mpLayer)

mpSubScaleY::mpSubScaleY(wxString name, int flags, bool ticks)
{
	SetName(name);
	SetFont( (wxFont&) *wxSMALL_FONT);
	SetPen( (wxPen&) *wxGREY_PEN);
...
void mpSubScaleY::Plot(wxDC & dc, mpWindow & w)
{
	if (m_visible) {
		dc.SetPen( m_pen);
		dc.SetFont( m_font);
		...
		for (;n < end; n += step) {
			...
			if ((p >= minYpx) && (p <= maxYpx)) {
				...
				dc.GetTextExtent(s, &tx, &ty);
#ifdef MATHPLOT_DO_LOGGING
				if (ty != labelHeigth) wxLogMessage(wxT("mpSubScaleY::Plot: ty(%f) and labelHeigth(%f) differ!"), ty, labelHeigth);
#endif
				labelW = (labelW <= tx) ? tx : labelW;
				if ((tmp-p+labelHeigth/2) > mpMIN_Y_AXIS_LABEL_SEPARATION) {
					if ((m_flags == mpALIGN_BORDER_LEFT) || (m_flags == mpALIGN_RIGHT))
    ...
    : wxScrolledWindow( parent, id, pos, size, flag, wxT("mathplot") )
{
    ...
    m_popmenu.Append( mpID_CENTER,     LNG("[Mathplot] Center"),      LNG("[Mathplot] Center plot view to this position"));
    m_popmenu.Append( mpID_FIT,        LNG("[Mathplot] Fit"),         LNG("[Mathplot] Set plot view to show all items"));
    m_popmenu.Append( mpID_ZOOM_IN,    LNG("[Mathplot] Zoom in"),     LNG("[Mathplot] Zoom in plot view."));
	m_popmenu.Append( mpID_ZOOM_OUT,   LNG("[Mathplot] Zoom out"),    LNG("[Mathplot] Zoom out plot view."));
	m_popmenu.Append( mpID_Previous,   LNG("[Mathplot] Previous"),    LNG("[Mathplot] Show Previous extend."));
    m_popmenu.AppendCheckItem( mpID_LOCKASPECT, LNG("[Mathplot] Lock aspect"), LNG("[Mathplot] Lock horizontal and vertical zoom aspect."));
    m_popmenu.Append( mpID_HELP_MOUSE,   LNG("[Mathplot] Show mouse commands..."),    LNG("[Mathplot] Show help about the mouse commands."));
	//edit by nowen
// 	m_popmenu.Append( mpID_CENTER,     wxT("����"),      wxT("ͼ�������λ�þ���"));
// 	m_popmenu.Append( mpID_FIT,        wxT("��Ӧ����"),         wxT("�ı�ͼ�δ�С������ʾ����ͼ��"));
// 	m_popmenu.Append( mpID_ZOOM_IN,    wxT("�Ŵ�"),     wxT("�Ŵ�ͼ��"));
// 	m_popmenu.Append( mpID_ZOOM_OUT,   wxT("��С"),    wxT("��Сͼ��"));
// 	m_popmenu.AppendCheckItem( mpID_LOCKASPECT, wxT("��������"), wxT("����ͼ�εĴ�ֱ��ˮƽ����"));
// 	m_popmenu.Append( mpID_HELP_MOUSE,   wxT("����������.."),    wxT("��ʾ����������"));
...
// 	}
		...
		//�޸ĳ�X���Y���ϵ�����,edit by nowen
		wxPoint clickPt( event.GetX(),event.GetY() );
		if (event.m_shiftDown)//����SHIFTΪx�᷽������
		{
			if (event.GetWheelRotation()>0)
				ZoomInX( clickPt );
			else    ZoomOutX( clickPt );
		}
		else//����Ϊy�᷽������
		{
			if (event.GetWheelRotation()>0)
				ZoomInY( clickPt );
			else    ZoomOutY( clickPt );
		}
    }
	UpdateAll();
}
...
#ifdef MATHPLOT_DO_LOGGING
	wxLogMessage(_("mpWindow::ZoomInX() prior coords: (%f,%f), new coords: (%f,%f) SHOULD BE EQUAL!!"), prior_layer_x,prior_layer_y, p2x(c.x),p2y(c.y));
#endif
...
#ifdef MATHPLOT_DO_LOGGING
	wxLogMessage(_("mpWindow::ZoomInY() prior coords: (%f,%f), new coords: (%f,%f) SHOULD BE EQUAL!!"), prior_layer_x,prior_layer_y, p2x(c.x),p2y(c.y));
#endif
...
#ifdef MATHPLOT_DO_LOGGING
	wxLogMessage(_("mpWindow::ZoomOutX() prior coords: (%f,%f), new coords: (%f,%f) SHOULD BE EQUAL!!"), prior_layer_x,prior_layer_y, p2x(c.x),p2y(c.y));
#endif
	UpdateAll();
}
...
#ifdef MATHPLOT_DO_LOGGING
	wxLogMessage(_("mpWindow::ZoomOutY() prior coords: (%f,%f), new coords: (%f,%f) SHOULD BE EQUAL!!"), prior_layer_x,prior_layer_y, p2x(c.x),p2y(c.y));
#endif
	UpdateAll();
}
...
//nowen
void mpWindow::OnShowPrevious(wxCommandEvent& WXUNUSED(event))
{
	ShowPrevious();
}

void mpWindow::On_Key_Down(wxKeyEvent &event)
{
	...
	case WXK_UP:
		...
		break;
	case WXK_DOWN:
		WindowMove(MOVE_DOWN);
		break;
	case WXK_LEFT:
		WindowMove(MOVE_LEFT);
		break;
	case WXK_RIGHT:
		WindowMove(MOVE_RIGHT);
		break;
...
{
    //nowen
	m_minX_previous = m_minX;
	m_maxX_previous = m_maxX;
	m_minY_previous = m_minY;
	m_maxY_previous = m_maxY;
